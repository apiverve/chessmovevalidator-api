...
class ChessmovevalidatorValidationException implements Exception {
  final List<String> errors;

  ChessmovevalidatorValidationException(this.errors);

  @override
  String toString() => 'ChessmovevalidatorValidationException: ${errors.join("; ")}';
}
...
/// Chess Move Validator API client.
///
/// For more information, visit: https://apiverve.com/marketplace/chessmovevalidator?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [move] (required) - Chess move in algebraic notation
class ChessmovevalidatorClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'move': ValidationRule(type: 'string', required: true),
  };

  ChessmovevalidatorClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/chessmovevalidator',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [ChessmovevalidatorValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw ChessmovevalidatorValidationException(errors);
    }
  }

  /// Execute a request to the Chess Move Validator API.
  ///
  /// Parameters are validated before sending the request.
  Future<ChessmovevalidatorResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw ChessmovevalidatorException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return ChessmovevalidatorResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw ChessmovevalidatorException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw ChessmovevalidatorException('Resource not found');
      } else {
        throw ChessmovevalidatorException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is ChessmovevalidatorException) rethrow;
      throw ChessmovevalidatorException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Chess Move Validator API client.
class ChessmovevalidatorException implements Exception {
  final String message;

  ChessmovevalidatorException(this.message);

  @override
  String toString() => 'ChessmovevalidatorException: $message';
}
